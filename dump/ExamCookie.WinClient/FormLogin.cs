...
namespace ExamCookie.WinClient
{
	...
	public class FormLogin : Form
	{
		...
		internal virtual LinkLabel BtnTerms
		{
			...
			set
			{
				LinkLabelLinkClickedEventHandler linkLabelLinkClickedEventHandler = new LinkLabelLinkClickedEventHandler(this, FormLogin.BtnTerms_LinkClicked);
		...
		internal virtual RadioButton TabManuelLogin
		{
			...
			set
			{
				EventHandler eventHandler = new EventHandler(this, FormLogin.TabChanged);
		...
		internal virtual Timer TmrAction
		{
			...
			set
			{
				EventHandler eventHandler = new EventHandler(this, FormLogin.OnTmrAction);
		...
		internal virtual WebView2 WebView1
		{
			...
			set
			{
				EventHandler<CoreWebView2NavigationCompletedEventArgs> eventHandler = new EventHandler<CoreWebView2NavigationCompletedEventArgs>(this, FormLogin.WebView1_NavigationCompleted);
				EventHandler<CoreWebView2InitializationCompletedEventArgs> eventHandler1 = new EventHandler<CoreWebView2InitializationCompletedEventArgs>(this, FormLogin.WebView1_CoreWebView2InitializationCompleted);
		...
		public FormLogin()
		{
			base.add_Load(new EventHandler(this, FormLogin.FormLogin_Load));
			base.add_FormClosing(new FormClosingEventHandler(this, FormLogin.FormLogin_FormClosing));
			base.add_Resize(new EventHandler(this, FormLogin.FormLogin_Resize));
		...
		private Int32 ClientSignIn(String examcode, String username, String password, ref String result)
		{
			...
			try
			{
				...
				try
				{
					...
					str = javaScriptSerializer.Serialize(new ParameterIn(Module1.GetOperatingSystem(), checked((Int32)Math.Round(Conversion.Val(this.VmDetect()))), eClientType.WINDOWS, Module1.GetFileversion()));
		...
		private Void FormLogin_Load(Object sender, EventArgs e)
		{
			...
			this.SetTimerAction(FormLogin.eAction.INIT, 0, false);
		...
		public String GetUniLoginUrl(String redirectUrl)
		{
			...
			try
			{
				...
				try
				{
					ExamApiV3Client examApiV3Client1 = examApiV3Client;
					Module1.SetCredentials(ref examApiV3Client1);
					webLoginUrl = examApiV3Client.GetWebLoginUrl(eWebLoginType.UNI_C, redirectUrl);
				}
				finally
				{
					if (examApiV3Client != null)
		...
		{
			this.TmrAction.Stop();
			this.TmrAction.set_Interval(100);
			FormLogin.eAction timerAction = this.TimerAction;
			switch (timerAction)
			{
				...
				default:
				{
					switch (timerAction)
					{
						...
						default:
						{
							if (timerAction == FormLogin.eAction.TEST_OS)
							{
		...
		{
			if (Operators.CompareString(uri.GetQueryString("auth"), "", false) != 0 & Operators.CompareString(uri.GetQueryString("timestamp"), "", false) != 0 & Operators.CompareString(uri.GetQueryString("user"), "", false) != 0 & Operators.CompareString(uri.GetQueryString("ltoken"), "", false) != 0)
			{
				user = uri.GetQueryString("user");
				password = String.Concat(new String[] { uri.GetQueryString("timestamp"), ".", uri.GetQueryString("auth"), ".", uri.GetQueryString("ltoken") });
			}
		}
		...
		private Void SetHtmlMessage(Object icon, String text = "", String confirm = "")
		{
			...
			{
				text = text.Replace("\r\n", "<br />");
				this.WebView1.CoreWebView2.Navigate(String.Concat(new Object[] { this.GetClientUIPage("message"), "?icon=", icon, "&text=", text, "&confirm=", confirm }));
			}
		}

		private Void SetTimerAction(FormLogin.eAction action, Int32 interval, Boolean start = false)
		{
			this.TmrAction.Stop();
		...
		private Void TabChanged(Object sender, EventArgs e)
		{
			...
			if (Operators.ConditionalCompareObjectEqual(NewLateBinding.LateGet(sender, null, "Checked", new Object[0], null, null, null), true, false))
			{
				...
				{
					this.SetTimerAction(FormLogin.eAction.MAN_LOGIN_BEGIN, 0, false);
					this.WebView1.CoreWebView2.Navigate(this.GetClientUIPage("manlogin"));
				}
		...
		private Void TryUniLoginTab(Uri uri)
		{
			if (this.TimerAction == FormLogin.eAction.UNI_LOGIN_IN_PROGRESS)
			{
				...
				String str1 = "";
				String str2 = "";
				this.ParseUniLoginResponse(uri, ref str1, ref str2);
				if (Operators.CompareString(str2, "", false) != 0)
				{
					num = this.ClientSignIn("", str1, str2, ref str);
					if (num != 0)
					{
		...
		private Void WebView1_NavigationCompleted(Object sender, CoreWebView2NavigationCompletedEventArgs e)
		{
			...
			else if (str.IndexOf("azurewebsites.net/index.html") > 0)
			{
				...
				else if (this.TimerAction == FormLogin.eAction.UNI_LOGIN_FAILED)
				{
			...
			else if (str.IndexOf("unilogin.dk/auth/realms/broker") > 0)
			{
				...
				{
					this.SetTimerAction(FormLogin.eAction.UNI_LOGIN_BEGIN, 0, false);
					this.WebView1.set_Visible(true);
			...
			else if (str.IndexOf("uni-login.dk/unilogin/logout") > 0)
			{
				if (this.TimerAction != FormLogin.eAction.UNI_LOGIN_BEGIN)
				{
					this.SetTimerAction(FormLogin.eAction.UNI_LOGIN_BEGIN, 0, false);
		...
		private enum eAction
		{
			...
			MAN_LOGIN_SUCCESS = 3,
			UNI_LOGIN_BEGIN = 10,
			UNI_LOGIN_IN_PROGRESS = 11,
			UNI_LOGIN_SUCCESS = 13,
			UNI_LOGIN_FAILED = 14,
			UNI_LOGIN_RESTART = 15,
			TEST_OS = 20,
			TEST_HD = 21,
			TEST_WEB_SERVICE = 22,
			TEST_CLIENT_VERSION = 23
		}
	}
}